...
UiManagers::UiManagers(const screenSettings &settings) : settings(settings) {}
...
void UiManagers::gameDisplay() {
  ...
  mvwprintw(stdscr, settings.startY, settings.startX + settings.boxWidth - 1,
            "+");
  mvwprintw(stdscr, settings.startY + settings.boxHeight - 1, settings.startX,
            "+");
  mvwprintw(stdscr, settings.startY + settings.boxHeight - 1,
            settings.startX + settings.boxWidth - 1, "+");
}
...
void UiManagers::scoreDisplay(int scores[]) {
  for (int i = 0; i < 2; i++) {
    mvwprintw(stdscr, settings.startY - 2 + i, settings.startX, "Score %d: %d",
              i + 1, scores[i]);
  }
}
